...
max_val = args[1]
min_val = args[2]
outdir = paste("biotype",max_val,min_val, sep = "_")
...
## I purposely used header=F because if I turn it on you see it alters the names of the lanes and it will no
## longer match the names in the other file, annoying but an easy fix!
...
write.table(data.frame(gene = names(java_gene_frequencies),
                       java = java_gene_frequencies,
                       parab = parab_gene_frequencies), file = "freqs_per_biotype.csv",
...
specific_to_java = names(java_gene_frequencies)[which(java_gene_frequencies >= max_val & parab_gene_frequencies <= min_val)]
specific_to_parab = names(parab_gene_frequencies)[which(parab_gene_frequencies >= max_val & java_gene_frequencies <= min_val)]
...
for (curr_gff_file in gff_files) {
  ...
  if (length(interesting_contigs) > 0) {
    ...
    p = ggplot(curr_df, aes(x = index, y = specific)) + geom_bar(stat = "identity", color = "black", size = 0.1) +
      ...
      geom_hline(yintercept = c(min_val,max_val,1), lty = 2)
...
write.table(summary_java, file.path(outdir,"summary_specific_java.csv"), col.names = T, row.names = F, quote = F, sep = ",")
write.table(summary_paraB, file.path(outdir,"summary_specific_paraB.csv"), col.names = T, row.names = F, quote = F, sep = ",")
